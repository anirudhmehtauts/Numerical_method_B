...
def lookback_option_price_and_greeks_surface(df, S, K, T, option_type, initial_date, n_paths=10000, n_steps=252,
                                          bump=0.01):
    ...
    def lookback_payoff(paths, strike):
        maximums = np.max(paths, axis=1)  # Average across time steps
        if option_type.lower() == 'call':
            return np.maximum(maximums - strike, 0)
        return np.maximum(strike - maximums, 0)

    base_paths = simulate_paths_with_surface(df, S, K, T, n_paths, n_steps, initial_date)
    base_payoffs = lookback_payoff(base_paths, K)
    ...
    up_payoffs = lookback_payoff(up_paths, K)
    down_payoffs = lookback_payoff(down_paths, K)
    ...
    payoffs_vol_up = lookback_payoff(paths_vol_up, K)
    payoffs_vol_down = lookback_payoff(paths_vol_down, K)

    price_vol_up = final_B * np.max(payoffs_vol_up)
    price_vol_down = final_B * np.max(payoffs_vol_down)
...
def calculate_pnl(df, option_params):
    ...
    if option_style == 'european':
        ...
        current_value, prev_delta, prev_gamma, prev_vega = lookback_option_price_and_greeks_surface(
    ...
    for date in dates:
        ...
        if option_style == 'european':
            ...
            next_value, delta, gamma, vega = lookback_option_price_and_greeks_surface(
...
def main():
    ...
    lookback_options = option_params[option_params['OptionType'] == 'Lookback']

    if lookback_options.empty:
        print("No lookback options found in the input data.")
        return
    ...
    for idx, option in lookback_options.iterrows():
        print(f"\nProcessing lookback option")
            ...
            for month in tqdm(months, desc=f"Processing months for year {year}", leave=False):
                ...
                if not df_month.empty:
                    ...
                    if not pnl_results.empty:
                        ...
                        filename = f"{year_dir}/pnl_series_lookback_strike_{option['Strike']}_{month}.csv"
